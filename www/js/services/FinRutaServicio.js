...
function ObtenerDocumentosParaControlDeFinDeRuta(callback, errorCallback) {
    try {
        var consultas = [{
            ...
            {
                consulta: "SELECT CLIENT_ID AS DOC_NO, 'Scouting' AS DOC_TYPE, IS_POSTED AS DOC_STATUS" +
            ...
            {
                consulta: "SELECT PAYMENT_NUM AS DOC_NO, 'Pago' AS DOC_TYPE, IS_POSTED AS DOC_STATUS" +
                    ", CASE WHEN SERVER_POSTED_DATETIME IS NULL THEN 'Pendiente' ELSE strftime('%H:%M:%S',SERVER_POSTED_DATETIME) END AS DOC_POSTED_DATE" +
                    ", DOC_SERIE, DOC_NUM " +
            ...
            {
                consulta: "SELECT CASE WHEN SALES_ORDER_ID_BO == 0 THEN SALES_ORDER_ID " +
                    "WHEN SALES_ORDER_ID_BO IS NULL THEN 'Error de secuencia' " +
            ...
            {
                consulta: "SELECT CUSTOMER_CHANGE_ID AS DOC_NO, 'Cambio Cliente' AS DOC_TYPE, IS_POSTED AS DOC_STATUS" +
                    ", CASE WHEN SERVER_POSTED_DATETIME IS NULL THEN 'Pendiente' ELSE strftime('%H:%M:%S',SERVER_POSTED_DATETIME) END AS DOC_POSTED_DATE " +
            ...
            {
                consulta: "SELECT CASE WHEN TAKE_INVENTORY_ID_BO = 0 THEN TAKE_INVENTORY_ID ELSE TAKE_INVENTORY_ID_BO END AS DOC_NO, 'Toma Inventario' AS DOC_TYPE, IS_POSTED AS DOC_STATUS" +
                    ", CASE WHEN SERVER_POSTED_DATETIME IS NULL THEN 'Pendiente' ELSE strftime('%H:%M:%S',SERVER_POSTED_DATETIME) END AS DOC_POSTED_DATE" +
            ...
            {
                consulta: "SELECT DOC_NUM AS DOC_NO" +

...
function ObtenerProductosDeOrdenesDeVenta(callback, errCallBack) {
  SONDA_DB_Session.transaction(
    function(tx) {
      var sql = "SELECT";
      sql += " d.SKU, s.SKU_NAME, SUM(d.QTY) QTY, SUM(d.TOTAL_AMOUNT_DISPLAY) TOTAL";
...
function ObtenerClientesDeOrdenesDeVenta(callback, errCallBack) {
  SONDA_DB_Session.transaction(
    function(tx) {
      var sql = "SELECT";
      sql += " h.SALES_ORDER_ID, c.CLIENT_ID, c.CLIENT_NAME, h.TOTAL_AMOUNT, h.TOTAL_AMOUNT_DISPLAY";

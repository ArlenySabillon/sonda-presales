                ...
                function(tx, results) {
                    for (var i = 0; i < results.rows.length; i++) {
                        var cliente = {
                            ...
                            SyncId: results.rows.item(i).SYNC_ID === "null" ||
                                results.rows.item(i).SYNC_ID === null ||
                                results.rows.item(i).SYNC_ID === "undefined" ?
                                gCurrentRoute +
                                fecha[0] +
                                fecha[1] +
                                results.rows.item(i).CLIENT_ID : results.rows.item(i).SYNC_ID,
...
///*********************ENVIO DE ORDES DE COMPRA *********************/
{
    ...
    function EnviarOrdenDeVenta(ordenesdeCompra, index, esUltimo) {
        ...
        this.ActualizarEnvioDeOrdernesDeCompra(data.salesOrder, function(dataN1) {}, function(err) {
